...
import com.damac.models.Assignment;
import com.damac.models.AssignmentApproval;
...
//			data.setPcc(false);
			...
			try {
				...
				WebResource webResource = client.resource(REST_URI).path("approval").path("assignment");
				ClientResponse response = webResource.type(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).post(ClientResponse.class, data);
				if (response.getStatus() == 201) {
					result = response.getEntity(AssignmentApproval.class);
